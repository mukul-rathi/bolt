...
let test_error_if_duplicate_class_defns () =
  ...
  in
  let example_class =
    Parsing.Parsed_ast.TClass (Class_name.of_string "Foo", None, None, [], [], []) in
  let result =
    get_instantiated_class_defn (Class_name.of_string "Foo")
...
let test_error_if_duplicate_class_fields () =
  ...
  let example_class =
    Parsing.Parsed_ast.TClass
      (Class_name.of_string "Foo", None, None, [], [example_field; example_field], [])
  in
  let result =
    get_class_field (Field_name.of_string "Baz") [example_class] example_class None
      Lexing.dummy_pos in
  Alcotest.(check string) "same error string" expected_error (print_error_string result)
