...
let check_no_duplicate_class_names class_defns =
  if
    List.contains_dup
      ~compare:
        (fun (Parsing.Parsed_ast.TClass (name_1, _, _, _, _))
             (Parsing.Parsed_ast.TClass (name_2, _, _, _, _)) ->
...
let type_method_defn class_defns function_defns
    ( Parsing.Parsed_ast.TClass (class_name, _, class_capabilities, _, _) as
    current_class_defn )
...
let type_class_defn
    ( Parsing.Parsed_ast.TClass
        (class_name, maybe_generic, capabilities, class_fields, method_defns) as
    current_class_defn ) class_defns function_defns =
  let open Result in
