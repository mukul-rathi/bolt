...
let rec free_vars_expr = function
  ...
  | Block (loc, block_type, block_exprs) -> (
    ...
    | []            -> []
    | expr :: exprs -> (
        free_vars_expr (Block (loc, block_type, exprs))
        |> fun exprs_free_vars ->
        match expr with
        ...
        | _ ->
            free_vars_expr expr
            |> fun expr_free_vars ->
            union_free_vars_lists [expr_free_vars; exprs_free_vars] ) )
  ...
  | FinishAsync (_, _, async_exprs, curr_thread_expr) ->
      union_free_vars_lists (List.map ~f:free_vars_expr (curr_thread_expr :: async_exprs))
  | If (_, _, cond_expr, then_expr, else_expr) ->
      union_free_vars_lists (List.map ~f:free_vars_expr [cond_expr; then_expr; else_expr])
  | While (_, cond_expr, loop_expr) ->
      union_free_vars_lists (List.map ~f:free_vars_expr [cond_expr; loop_expr])
  | For (loc, start_expr, cond_expr, step_expr, loop_expr) ->
      (* desugar into a while loop *)
      free_vars_expr
        (Block (loc, TEVoid, [start_expr; While (loc, cond_expr, loop_expr); step_expr]))
