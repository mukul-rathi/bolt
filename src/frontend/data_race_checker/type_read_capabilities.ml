...
let remove_read_capabilities id =
  let filtered_capabilities =
    List.filter
      ~f:(fun (TCapability (mode, _)) -> not (mode = Read))
      (get_identifier_capabilities id) in
  set_identifier_capabilities id filtered_capabilities

let rec type_read_capabilities_expr expr =
  match expr with
  | Integer _ | Boolean _ -> expr
  | Identifier (loc, id) -> Identifier (loc, id)
  | BlockExpr (loc, block_expr) ->
      BlockExpr (loc, type_read_capabilities_block_expr block_expr)
  | Constructor (loc, type_expr, class_name, constructor_args) ->
      let updated_args =
        List.map
          ~f:(fun (ConstructorArg (type_expr, field_name, expr)) ->
            ConstructorArg (type_expr, field_name, type_read_capabilities_expr expr))
          constructor_args in
      Constructor (loc, type_expr, class_name, updated_args)
  | Let (loc, type_expr, var_name, bound_expr) ->
      Let (loc, type_expr, var_name, type_read_capabilities_expr bound_expr)
  ...
  | FunctionApp (loc, return_type, func_name, args) ->
      FunctionApp
        (loc, return_type, func_name, List.map ~f:type_read_capabilities_expr args)
  | Printf (loc, format_str, args) ->
      Printf (loc, format_str, List.map ~f:type_read_capabilities_expr args)
      ...
      FinishAsync
        ...
        , List.map
            ~f:(fun (AsyncExpr (free_vars, expr)) ->
              AsyncExpr (free_vars, type_read_capabilities_block_expr expr))
            async_exprs
  ...
  | UnOp (loc, type_expr, unop, expr) ->
      UnOp (loc, type_expr, unop, type_read_capabilities_expr expr)

and type_read_capabilities_block_expr (Block (loc, type_block_expr, exprs)) =
  let updated_exprs = List.map ~f:type_read_capabilities_expr exprs in
