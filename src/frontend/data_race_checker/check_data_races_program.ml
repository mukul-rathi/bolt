...
let check_data_races_block_expr class_defns block_expr =
  let open Result in
...
let check_data_races_class_defn class_defns
    (Typing.Typed_ast.TClass (class_name, regions, fields, method_defns)) =
  let open Result in
  Result.all (List.map ~f:(check_data_races_method_defn class_defns) method_defns)
  >>| fun data_race_checked_method_defns ->
  Data_race_checker_ast.TClass
    (class_name, regions, fields, data_race_checked_method_defns)

let check_data_races_program
    (Typing.Typed_ast.Prog (class_defns, function_defns, main_expr)) =
  let open Result in
  Result.all (List.map ~f:(check_data_races_class_defn class_defns) class_defns)
  >>= fun data_race_checked_class_defns ->
  Result.all (List.map ~f:(check_data_races_function_defn class_defns) function_defns)
  >>= fun data_race_checked_function_defns ->
  check_data_races_block_expr class_defns main_expr
