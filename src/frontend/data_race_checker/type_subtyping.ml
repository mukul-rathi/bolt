...
let type_subtyping_capabilities class_defns class_name superclass =
  ...
  Result.all_unit
    (List.map
       ~f:(fun superclass_cap1 ->
         Result.all_unit
           (List.map
              ~f:(fun superclass_cap2 ->
                if
                  (* if capabilities can be used concurrently in the superclass they must
                     also be able to be used concurrently in subclasses *)
                  (not
                     (can_concurrently_access_capabilities superclass class_defns
