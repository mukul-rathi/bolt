...
let get_class_defn class_name class_defns =
  let matching_class_defns =
    List.filter
      ~f:(fun (Typing.Typed_ast.TClass (name, _, _, _, _, _)) -> class_name = name)
      class_defns in
...
let rec get_class_method_defns class_name class_defns =
  ...
  |> fun (Typing.Typed_ast.TClass (_, _, maybe_inherits, _, _, method_defns)) ->
  ...
  |> List.dedup_and_sort
       ~compare:(fun (Typing.Typed_ast.TMethod (name_1, _, _, params_1, _, _))
                     (Typing.Typed_ast.TMethod (name_2, _, _, params_2, _, _))
                     ->
...
let rec get_class_capabilities class_name class_defns =
  get_class_defn class_name class_defns
  |> fun (Typing.Typed_ast.TClass (_, _, maybe_inherits, capabilities, _, _)) ->
  ( match maybe_inherits with
...
let rec get_class_fields class_name class_defns =
  get_class_defn class_name class_defns
  |> fun (Typing.Typed_ast.TClass (_, _, maybe_inherits, _, field_defns, _)) ->
  ( match maybe_inherits with
...
let maybe_get_superclass class_name class_defns =
  get_class_defn class_name class_defns
  |> fun (Typing.Typed_ast.TClass (_, _, maybe_inherits, _, _, _)) -> maybe_inherits
