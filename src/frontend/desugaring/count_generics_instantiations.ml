...
let rec count_generics_instantiations_expr expr class_insts =
  match expr with
  | Typed_ast.Integer _ | Typed_ast.Boolean _ | Typed_ast.Identifier _ -> class_insts
  | Typed_ast.BlockExpr (_, block_expr) ->
      count_generics_instantiations_block_expr block_expr class_insts
  | Typed_ast.Constructor (_, class_name, maybe_type_param, constructor_args) ->
      ...
      List.fold ~init:updated_class_insts
        ~f:(fun acc_class_insts (Typed_ast.ConstructorArg (_, _, arg_expr)) ->
          count_generics_instantiations_expr arg_expr acc_class_insts)
        constructor_args
  | Typed_ast.Let (_, _, _, bound_expr) ->
      count_generics_instantiations_expr bound_expr class_insts
  | Typed_ast.Assign (_, _, _, assigned_expr) ->
      count_generics_instantiations_expr assigned_expr class_insts
  ...
  | Typed_ast.FinishAsync (_, _, async_exprs, curr_thread_expr) ->
      count_generics_instantiations_block_expr curr_thread_expr class_insts
      |> fun updated_class_insts ->
      List.fold ~init:updated_class_insts
        ~f:(fun acc_class_insts (Typed_ast.AsyncExpr async_expr) ->
          count_generics_instantiations_block_expr async_expr acc_class_insts)
        async_exprs
  | Typed_ast.If (_, _, cond_expr, then_expr, else_expr) ->
      count_generics_instantiations_expr cond_expr class_insts
      |> fun updated_class_insts ->
      List.fold ~init:updated_class_insts
        ~f:(fun acc_class_insts branch_expr ->
          count_generics_instantiations_block_expr branch_expr acc_class_insts)
        [then_expr; else_expr]
  | Typed_ast.While (_, cond_expr, loop_expr) ->
      count_generics_instantiations_expr cond_expr class_insts
      |> fun updated_class_insts ->
      count_generics_instantiations_block_expr loop_expr updated_class_insts
  | Typed_ast.BinOp (_, _, _, expr1, expr2) ->
      List.fold ~init:class_insts
        ~f:(fun acc_class_insts op_expr ->
          count_generics_instantiations_expr op_expr acc_class_insts)
        [expr1; expr2]
  | Typed_ast.UnOp (_, _, _, op_expr) ->
      count_generics_instantiations_expr op_expr class_insts

and count_generics_instantiations_block_expr (Typed_ast.Block (_, _, exprs)) class_insts =
  List.fold ~init:class_insts
    ~f:(fun acc_class_insts expr ->
      count_generics_instantiations_expr expr acc_class_insts)
    exprs

let count_generics_instantiations_function_defn
    (Typed_ast.TFunction (_, _, _, _, body_expr)) class_insts =
  count_generics_instantiations_block_expr body_expr class_insts

let count_generics_instantiations_class_defn (Typed_ast.TClass (_, _, _, _, method_defns))
    class_insts =
  List.fold ~init:class_insts
    ~f:(fun acc_class_insts (Typed_ast.TMethod (_, _, _, _, _, body_expr)) ->
      count_generics_instantiations_block_expr body_expr acc_class_insts)
    method_defns

let count_generics_instantiations_program
    (Typed_ast.Prog (class_defns, function_defns, main_expr)) =
  count_generics_instantiations_block_expr main_expr []
  |> fun class_insts ->
  List.fold ~init:class_insts
    ~f:(fun acc_class_insts class_defn ->
      count_generics_instantiations_class_defn class_defn acc_class_insts)
    class_defns
  |> fun updated_class_ints ->
  List.fold ~init:updated_class_ints
    ~f:(fun acc_class_insts function_defn ->
      count_generics_instantiations_function_defn function_defn acc_class_insts)
    function_defns
