...
let desugar_identifier class_defns borrowed_vars id =
  ...
  | Typed_ast.ObjField (class_name, _, obj_name, field_type, field_name) ->
...
let rec desugar_expr class_defns function_defns borrowed_vars expr =
  ...
  match expr with
  | Typed_ast.Integer (loc, i) -> Desugared_ast.Integer (loc, i)
  | Typed_ast.Boolean (loc, b) -> Desugared_ast.Boolean (loc, b)
  ...
  | Typed_ast.Constructor (loc, class_name, _, constructor_args) ->
      List.map
        ~f:(fun (Typed_ast.ConstructorArg (type_expr, field_name, expr)) ->
  ...
  | Typed_ast.MethodApp
      ...
      Desugared_ast.MethodApp
        ...
        , desugared_args )
  | Typed_ast.FunctionApp (loc, type_expr, func_params, func_name, args) ->
      List.map ~f:(desugar_expr class_defns function_defns borrowed_vars) args
      |> fun desugared_args ->
  ...
  | Typed_ast.UnOp (loc, type_expr, un_op, expr) ->
      desugar_expr class_defns function_defns borrowed_vars expr
...
and desugar_async_expr class_defns function_defns borrowed_vars
    (Typed_ast.AsyncExpr async_block_expr) =
