...
let rec desugar_expr class_defns borrowed_vars expr =
  ...
  | Typing.Typed_ast.BlockExpr (loc, block_expr) ->
      desugar_block_expr class_defns borrowed_vars block_expr
  ...
  | Typing.Typed_ast.Constructor (loc, type_expr, class_name, constructor_args) ->
      ...
      List.map
        ~f:(fun (Typing.Typed_ast.ConstructorArg (type_expr, field_name, expr)) ->
          desugar_expr class_defns borrowed_vars expr
          |> fun desugared_expr ->
  ...
  | Typing.Typed_ast.Let (loc, type_expr, var_name, bound_expr) ->
      desugar_expr class_defns borrowed_vars bound_expr
  ...
  | Typing.Typed_ast.Assign (loc, type_expr, id, assigned_expr) ->
      desugar_expr class_defns borrowed_vars assigned_expr
  ...
  | Typing.Typed_ast.FinishAsync (loc, type_expr, async_exprs, curr_thread_expr) ->
      ...
      List.map ~f:(desugar_async_expr class_defns borrowed_vars) async_exprs
      |> fun desugared_async_exprs ->
      desugar_block_expr class_defns borrowed_vars curr_thread_expr
  ...
  | Typing.Typed_ast.If (loc, type_expr, cond_expr, then_expr, else_expr) ->
      desugar_expr class_defns borrowed_vars cond_expr
      |> fun desugared_cond_expr ->
      desugar_block_expr class_defns borrowed_vars then_expr
      |> fun desugared_then_expr ->
      desugar_block_expr class_defns borrowed_vars else_expr
  ...
  | Typing.Typed_ast.While (loc, cond_expr, loop_expr) ->
      desugar_expr class_defns borrowed_vars cond_expr
      |> fun desugared_cond_expr ->
      desugar_block_expr class_defns borrowed_vars loop_expr
  ...
  | Typing.Typed_ast.BinOp (loc, type_expr, bin_op, expr1, expr2) ->
      desugar_expr class_defns borrowed_vars expr1
      |> fun desugared_expr1 ->
      desugar_expr class_defns borrowed_vars expr2
      |> fun desugared_expr2 ->
...
and desugar_block_expr class_defns borrowed_vars
    (Typing.Typed_ast.Block (loc, type_expr, exprs)) =
  List.map ~f:(desugar_expr class_defns borrowed_vars) exprs
  |> fun desugared_exprs -> Desugared_ast.Block (loc, type_expr, desugared_exprs)

and desugar_async_expr class_defns borrowed_vars
  ...
  desugar_block_expr class_defns borrowed_vars async_block_expr
