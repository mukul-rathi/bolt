...
let name_mangle_generics_id id =
  match id with
...
let rec name_mangle_generics_expr expr =
  match expr with
  | Typed_ast.Integer _ | Typed_ast.Boolean _ -> expr
  | Typed_ast.Identifier (loc, id) ->
      Typed_ast.Identifier (loc, name_mangle_generics_id id)
  | Typed_ast.BlockExpr (loc, block_expr) ->
      Typed_ast.BlockExpr (loc, name_mangle_generics_block_expr block_expr)
      ...
      List.map
        ~f:(fun (Typed_ast.ConstructorArg (type_expr, field_name, arg_expr)) ->
          Typed_ast.ConstructorArg
            ...
            , field_name
            , name_mangle_generics_expr arg_expr ))
        constructor_args
  ...
  | Typed_ast.Let (loc, type_expr, var_name, bound_expr) ->
      Typed_ast.Let
        ...
        , name_mangle_if_generic_type type_expr
        , var_name
        , name_mangle_generics_expr bound_expr )
  | Typed_ast.Assign (loc, type_expr, id, assigned_expr) ->
      Typed_ast.Assign
        ( loc
        , name_mangle_if_generic_type type_expr
        , name_mangle_generics_id id
        , name_mangle_generics_expr assigned_expr )
  | Typed_ast.Consume (loc, id) -> Typed_ast.Consume (loc, name_mangle_generics_id id)
  | Typed_ast.MethodApp
      ...
      List.map ~f:name_mangle_generics_expr args
      |> fun name_mangled_args ->
      Typed_ast.MethodApp
        ( loc
  ...
  | Typed_ast.FinishAsync (loc, type_expr, async_exprs, curr_thread_expr) ->
      List.map
        ~f:(fun (Typed_ast.AsyncExpr async_expr) ->
          Typed_ast.AsyncExpr (name_mangle_generics_block_expr async_expr))
        async_exprs
      ...
      Typed_ast.FinishAsync
        ...
        , name_mangle_generics_block_expr curr_thread_expr )
  | Typed_ast.If (loc, type_expr, cond_expr, then_expr, else_expr) ->
      Typed_ast.If
        ( loc
        , name_mangle_if_generic_type type_expr
        , name_mangle_generics_expr cond_expr
        , name_mangle_generics_block_expr then_expr
        , name_mangle_generics_block_expr else_expr )
  | Typed_ast.While (loc, cond_expr, loop_expr) ->
      Typed_ast.While
        ( loc
        , name_mangle_generics_expr cond_expr
        , name_mangle_generics_block_expr loop_expr )
  | Typed_ast.BinOp (loc, type_expr, bin_op, expr1, expr2) ->
      Typed_ast.BinOp
        ( loc
        , name_mangle_if_generic_type type_expr
        , bin_op
        , name_mangle_generics_expr expr1
        , name_mangle_generics_expr expr2 )
  | Typed_ast.UnOp (loc, type_expr, un_op, op_expr) ->
      Typed_ast.UnOp
        ( loc
        , name_mangle_if_generic_type type_expr
        , un_op
        , name_mangle_generics_expr op_expr )

and name_mangle_generics_block_expr (Typed_ast.Block (loc, type_expr, exprs)) =
  List.map ~f:name_mangle_generics_expr exprs
  |> fun name_mangled_exprs ->
  Typed_ast.Block (loc, name_mangle_if_generic_type type_expr, name_mangled_exprs)

let name_mangle_generics_field_defn
...
let name_mangle_generics_param
...
let instantiate_maybe_generic_method_defn
    ...
    =
  List.map ~f:name_mangle_generics_param params
  ...
  Typed_ast.TMethod
    ...
    , name_mangled_params
    , meth_cap_names
    , name_mangle_generics_block_expr body_expr )

let name_mangle_generics_class_defn
    (Typed_ast.TClass (class_name, _, caps, field_defns, method_defns)) =
  List.map ~f:name_mangle_generics_field_defn field_defns
  |> fun name_mangled_field_defns ->
  List.map ~f:instantiate_maybe_generic_method_defn method_defns
  |> fun name_mangled_method_defns ->
  Typed_ast.TClass
    (class_name, None, caps, name_mangled_field_defns, name_mangled_method_defns)

let name_mangle_generics_function_defn
    (Typed_ast.TFunction
...
let name_mangle_generics_program (Typed_ast.Prog (class_defns, function_defns, main_expr))
    =
  List.map ~f:name_mangle_generics_class_defn class_defns
  |> fun name_mangled_class_defns ->
  List.map ~f:name_mangle_generics_function_defn function_defns
  |> fun name_mangled_function_defns ->
  Typed_ast.Prog
    ( name_mangled_class_defns
    , name_mangled_function_defns
    , name_mangle_generics_block_expr main_expr )
