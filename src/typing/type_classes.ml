...
let check_req_field_present error_prefix
  ...
  then Ok ()
    ...
    Error
      (Error.of_string
         ( error_prefix ^ "missing required field:  "
...
let check_valid_cap_trait error_prefix (TCapTrait (capability, trait_name)) trait_defns =
  ...
  | []                  -> Error
                             (Error.of_string (error_prefix ^ "No matching declarations."))
  | [trait_defn] -> Ok trait_defn
  | _                   -> Error
                             (Error.of_string
                                (error_prefix ^ "Duplicate trait declarations."))
...
let check_no_duplicate_fields error_prefix field_defns =
  if List.contains_dup ~compare:(fun x y -> if x = y then 0 else 1) field_defns then
    Error (Error.of_string (error_prefix ^ "Duplicate field declarations."))
  else Ok ()
...
let type_class_defn
  ...
  let error_prefix = Class_name.to_string class_name ^ "has a type error: " in
